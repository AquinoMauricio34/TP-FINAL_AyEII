...
void pago_cuenta(turno_cliente **ini_turno_cliente,actividad *ini_actividad,tipo_turno *ini_tipo_turno,cuenta **ini_cuenta){
    ...
    do{
        ...
        buscar = buscar_dni_turno_cliente_cuenta(0,dni_cliente,*ini_turno_cliente);
    ...
    if(dni_cliente != 0){
        ...
        if(eleccion_sede != 0){
            ...
            if(eleccion_actividad != 0){
                ...
                do{
                    system("cls");
                    if(*ini_turno_cliente!=NULL){
                        printf("%-10s | %-13s | %-13s | %-10s | %-7s | %-17s | %-7s |\n","CODIGO","CODIGO","CODIGO","DNI","DEBE","F. ULTIMA","BAJA");
                        printf("%-10s | %-13s | %-13s | %-10s | %-7s | %-17s | %-7s |\n\n","TURNO","ACTIVIDAD","T. CLIENTE","","","VEZ","");
                    }
                    con_cuentas = listar_turnos_cliente_segunactividad(dni_cliente,eleccion_actividad,*ini_turno_cliente,0);
                    if(con_cuentas == 0){
                        printf("SIN CUENTAS DEL CLIENTE PARA ESTA ACTIVIDAD.\n");
                    }
                    if(buscar == -1){
                    ...
                    buscar = buscar_turno_cliente_cuenta(0,eleccion_turno_cliente,eleccion_actividad,dni_cliente,&aux_turno_cliente,*ini_turno_cliente);
...
cliente * insertar_cuenta(cuenta **nv, cuenta *ini_cuenta){
	...
	if(ini_cuenta != NULL)
		ini_cuenta->sgte = insertar_cuenta(&*nv,ini_cuenta->sgte);
	else
		ini_cuenta = *nv;
		*nv = NULL;
	return ini_cuenta;
...
int buscar_turno_cliente_cuenta(int buscar,int eleccion_turno_cliente,int eleccion_actividad,long int dni_cliente,turno_cliente **aux_turno_cliente,turno_cliente *ini_clientesta){
	if(ini_clientesta != NULL && buscar != 1 && buscar != -1){
		if(ini_clientesta->cod_clientesta == eleccion_turno_cliente && ini_clientesta->cod_act == eleccion_actividad && ini_clientesta->dni == dni_cliente){
			if(ini_clientesta->debe > 0)
				buscar = 1;
			else
		...
		buscar = buscar_turno_cliente_cuenta(buscar,eleccion_turno_cliente,eleccion_actividad,dni_cliente,&*aux_turno_cliente,ini_clientesta->sgte);
	}
	return buscar;
}
...
int buscar_dni_turno_cliente_cuenta(int encontrado,long int dni,turno_cliente *ini_turno_cliente){
	if(ini_turno_cliente != NULL && encontrado != 1){
		if(ini_turno_cliente->dni == dni){
			encontrado = 1;
		}
		encontrado = buscar_dni_turno_cliente_cuenta(encontrado,dni,ini_turno_cliente->sgte);
	}
	return encontrado;
}
...
int listar_turnos_cliente_segunactividad(long int dni_cliente,int eleccion_actividad,turno_cliente *ini,int con_cuentas){
	if(ini != NULL){
		...
		con_cuentas = listar_turnos_cliente_segunactividad(dni_cliente,eleccion_actividad,ini->sgte,con_cuentas);
	}
    return con_cuentas;
}
